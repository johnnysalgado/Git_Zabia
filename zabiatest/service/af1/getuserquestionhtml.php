...
function obtenerControlUnico($esSelect, $idControl, $respuestaArreglo, $requerido, $esPreguntaRequerida, $controlPresentacion, $clasePrimerControl, $ocultaOpcion, $lenguaje) {
    ...
    if ($esSelect) {
        ...
        $htmlControl = "<div data-field-is-required=\"$esPreguntaRequerida\" class=\"d-flex\">";
        $contadorRadio = 0; 
        foreach ($respuestaArreglo as $itemRespuesta) {
            $contadorRadio++;
            $idOpcion = $itemRespuesta['answerID'];
            ...
            $questionSkip = $itemRespuesta['questionSkip'];
            $answerNone = $itemRespuesta['answerNone'];
            $htmlControl .= " <div class=\"custom-control custom-radio ";
            if ($contadorRadio > 1) {
                $htmlControl .= " ml-2";
            }
            /*
            if ($controlPresentacion == CONTROL_PRESENTACION_HORIZONTAL) {
                $htmlControl .= " inline-block";
            }
            */
            $htmlControl .= " \">";
            $htmlControl .= " <input type=\"radio\" id=\"$idControlOpcion\" name=\"$idControl\" value=\"$idOpcion\" $checked data-section-show=\"$sectionShow\" data-section-hide=\"$sectionSkip\" data-question-show=\"$questionShow\" data-question-hide=\"$questionSkip\" data-none=\"$answerNone\" class=\"custom-control-input\" /> <label for=\"$idControlOpcion\" class=\"custom-control-label\"> $opcion </label>";
            $htmlControl .= " </div> ";
        }
        $htmlControl .= " </div>";
    }
    ...
    if ($esSelectParaLineal) {
        ...
        if ($controlPresentacion == CONTROL_PRESENTACION_VERTICAL) {
            ...
            foreach ($respuestaArreglo as $itemRespuesta) {
                ...
                $questionSkip = $itemRespuesta['questionSkip'];
                $answerNone = $itemRespuesta['answerNone'];
                $htmlControl .= "<div class=\"custom-control custom-checkbox\">";
                $htmlControl .= " <input  type=\"checkbox\" id=\"$idControlOpcion\" name=\"$idControl\" value=\"$idOpcion\" $checked data-section-show=\"$sectionShow\" data-section-hide=\"$sectionSkip\" data-question-show=\"$questionShow\" data-question-hide=\"$questionSkip\" data-none=\"$answerNone\" class=\"custom-control-input\" /> <label class=\"custom-control-label\" for=\"$idControlOpcion\"> $opcion </label> ";
                $htmlControl .= "</div>";
            }
            $htmlControl .= " </div>";
        } else {
            ...
            foreach ($respuestaArreglo as $itemRespuesta) {
                ...
                if ($userSelected == "1") {
                    ...
                    $htmlOpcionMulti .= "<tr> <td> $opcion </td> <td class=\"text-right\"> <a data-value=\"$idOpcion\" href=\"javascript:;\" class=\"btn-remove btn btn-danger\"> <i class=\"fa fa-trash\"></i> </a> </td> </tr> ";
                } else {
                    $style = "";
                }
                $sectionShow = $itemRespuesta['sectionShow'];
    ...
    while ($contadorArreglo < $cantidadArreglo) {
        ...
        while ($contadorArreglo < $cantidadArreglo && $idSeccionBD == $idSeccionBDx) {
            ...
            if ($tieneSubpregunta == 1) {
                $htmlPregunta .= " <div id=\"divQuestion_$idPreguntax\" class=\"col-md-12 mb-3 border rounded\" > <div class=\"row align-items-end\"> <div class=\"[CLASE_COL]\"> <label class=\"control-label\" > $tituloPregunta </label> ";
                ...
                $htmlPregunta .= " <div class=\"$colMd text-right\"> <a href=\"javascript:void(0)\" id=\"" . PREFIJO_CONTROL_PREGUNTA . "_" . $idPreguntax . "_btn\" class=\"btn-add btn btn-raised btn-primary \"> <i class=\"fas fa-plus-square\"></i> </a> </div> </div> <table id=\"table_$idControlPregunta" . "\" class=\"table mt-2\"> <tbody>";
                ...
                foreach ($usuarioSubpreguntaArreglo as $itemUsuario) {
                    ...
                    $htmlPregunta .= "<td class=\"$colTd text-right\"> <a data-value=\"$valuesp\" data-id=\"$xidUsuarioPreguntaSubpregunta\" href=\"javascript:;\" class=\"btn-remove btn btn-danger\"> <i class=\"fa fa-trash\"></i> </a> </td> </tr>";
                }
                ...
                $htmlPregunta .= " <div id=\"divQuestion_$idPreguntax\" class=\"col-md-$cols mb-3\" > <label class=\"control-label\">$tituloPregunta</label> $htmlRespuesta </div> ";
            }
        }
