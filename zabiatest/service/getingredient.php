...
if (isset($postdata)) {
    ...
    $idTipoAlimentoSet = $request->foodType;
    ...
    if (trim($idTipoAlimentoSet) != "") {
        $idTipoAlimentoSet = str_replace("|", ",", str_replace("'", "''", $idTipoAlimentoSet));
    }

    $arregloData = array();

    $daoInsumo = new DaoInsumo();
    $listaInsumo = $daoInsumo->listarInsumoSemaforoPorUsuario($idUsuario, $idTipoAlimentoSet, $busqueda, $lenguaje, $page, $records);
    ...
    foreach ($listaInsumo as $item) {
        ...
        $tipoAlimentoIngles = $item['tipo_alimento_ing'];
        $imagen = $item['imagen'];
        $nombreImagen = $imagen;
        $setImagenURL = "";
        if ($imagen != "") {
        ...
        array_push($arregloData, array('ingredientID' => $idInsumo, 'name' => $nombrePrincipal, 'alternativeName' => '', 'foodType' => $tipoAlimentoPrincipal, 'image' => $imagen, 'imageName' => $nombreImagen, 'imageSet' => $setImagenURL, 'imageSize' => SIZE_IMAGE_SET, 'semaphore' => $semaforoInsumoUsuarioArray));
    }
