...
if (isset($postdata)) {
    ...
    $arregloTipoAlimento = $daoInsumo->listarTipoAlimento(LISTA_ACTIVO, $lenguaje);
    foreach ($arregloTipoAlimento as $item) {
        $idTipoalimento = $item['id_tipo_alimento'];
        $imagen = "$idTipoalimento.jpg";
        $nombreImagen = $imagen;
        $setImagenURL = "";
        if ($imagen != "") {
        ...
        $nombre = $item['nombre'];
        $nombreIng = $item['nombre_ing'];
        $tipoAlimentoPrincipal = $nombre;
        if ($lenguaje == LENGUAJE_INGLES) {
            $tipoAlimentoPrincipal = $nombreIng;
        }

        array_push($arregloData, array('foodTypeID' => $item['id_tipo_alimento'], 'name' => $tipoAlimentoPrincipal, 'image' => $imagen, 'imageName' => $nombreImagen, 'imageSet' => $setImagenURL, 'imageSize' => SIZE_IMAGE_SET));
    }
