<?php
    ...
    if (isset($_POST['id_pregunta'])) {
        ...
        $idPreguntaTipoGenero = $_POST['id_pregunta_tipo_genero'];
        $orden = $_POST['orden'];
        $estado = $_POST['estado'];
        ...
        $daoCuestionario->editarPreguntaTipoGenero($idPreguntaTipoGenero, $orden, $none, $estado, $usuario);
        $daoCuestionario = null;
        header("Location: pregunta_tipo_genero.php?id_pregunta=$idPregunta&e=$estadoABuscar");
        die();
        ...
        $daoCuestionario = new DaoCuestionario();
        if ($obtenerNuevos == "1") {
            $daoCuestionario->poblarPreguntaTipoGenero($idPregunta, $usuario);
        }
        $arregloPreguntaTipoGenero = $daoCuestionario->listarPreguntaTipoGenero($idPregunta, $estadoABuscar);
        if (count($arregloPreguntaTipoGenero) == 0 && $estadoABuscar == 1) {
            $daoCuestionario->poblarPreguntaTipoGenero($idPregunta, $usuario);
            $arregloPreguntaTipoGenero = $daoCuestionario->listarPreguntaTipoGenero($idPregunta, LISTA_ACTIVO);
        }
        $html = "<ol class=\"dd-list\">";
        foreach ($arregloPreguntaTipoGenero as $item) {
            $idPreguntaTipoGenero = $item['id_pregunta_tipo_genero'];
            $descripcion = $item['nombre'] . " / " . $item['nombre_ing'];
            $estado = $item['estado'];
            $clase = ($estado == 1) ? CLASE_ACTIVO : CLASE_INACTIVO;
            $html .= "<li class=\"dd-item dd3-item\" data-id=\"$idPreguntaTipoGenero\"> <div class=\"dd-handle dd3-handle $clase\"> </div>  <div class=\"dd3-content\" data-id=\"$idPreguntaTipoGenero\"> <input type=\"checkbox\" name=\"chk_eliminar[]\" value=\"$idPreguntaTipoGenero\" /> <span style=\"cursor:pointer;\" class=\"ver-option\" data-id=\"$idPreguntaTipoGenero\"> $descripcion </span> </div> </li>";
        }
        $html .= "</ol>";
        $descripcionPregunta = "";
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-xs-6">
                            <div class="white-box">
                                <form action="pregunta_tipo_genero_eliminar.php" method="post" id="forma-eliminar">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 text-right">
                                            ...
                                            &nbsp;&nbsp;
                                            <input type="submit" id="eliminar-todo" value="<?php if ($estadoABuscar == LISTA_ACTIVO) { echo 'Eliminar'; } else { echo  'Activar'; } ?>" class="btn btn-primary" />
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="col-md-6 col-xs-8">
                            <div class="white-box">
                                <form action="pregunta_tipo_genero.php" method="post" id="forma">
                                    <div class="row">
                                        <div class="col-md-12">
                                    ...
                                    <input type="hidden" name="id_pregunta_tipo_genero" id="id_pregunta_tipo_genero" value="0" />
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var updateOutput = function(e) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: "service/savehealthgendertypeorder.php",
                        data: {
                ...
                $('.ver-option').click(function() {
                    limpiarForma();
                    var idPreguntaTipoGenero = $(this).attr('data-id');
                    var param = { "healthGenderTypeID" : idPreguntaTipoGenero };
                    var paramJSON = JSON.stringify(param);
                    $.ajax({
                        type: 'POST',
                        url: 'service/gethealthgendertypebyid.php',
                        data: paramJSON,
                        dataType: 'json',
                        ...
                        success: function (result) {
                            console.log(result);
                            $('#id_pregunta_tipo_genero').val(result.data[0].healthGenderTypeID);
                ...
                $('#listar-todo').click( function () {
                    ...
                    ?>
                    location.href = "pregunta_tipo_genero.php?id_pregunta=<?php echo $idPregunta; ?>&e=" + estadoABuscar;
                });

                $('#obtener-nuevo').click( function() {
                    location.href = "pregunta_tipo_genero.php?id_pregunta=<?php echo $idPregunta; ?>&e=<?php echo $estadoABuscar; ?>&on=1";
                });
